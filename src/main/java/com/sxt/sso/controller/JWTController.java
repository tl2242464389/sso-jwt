...
public class JWTController {
	...
	public Object testAll(HttpServletRequest request){
		...
		if(result.isSuccess()){
			...
			// 根据相同信息重新生成token，就是为了重置token的有效期。
			String newToken = JWTUtils.createJWT(result.getClaims().getId(), 
					result.getClaims().getIssuer(), result.getClaims().getSubject(), 1*60*1000);
	...
	public Object login(String username, String password){
		...
		if(JWTUsers.isLogin(username, password)){
			...
			String jwtToken = JWTUtils.createJWT(UUID.randomUUID().toString(), "iss-test",
